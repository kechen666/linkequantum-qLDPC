...
def vqa_optimization(noise_model, backend, theta_init, hamiltonian, shots=500):
    ...
    result = minimize(objective, theta_init, method='Nelder-Mead', \
                      options={'maxiter': 1001}, callback=callback)
...
def vqa_energy_sim(n_qubits):
    ...
    if n_qubits == 15:
        ...
        theta_init = [np.pi / 2 for i in range(15)]
    elif n_qubits == 10:
        theta_init = [0.82782756, 2.19374341, 2.70773908, 0.57133075, 0.76353896, 0.51599272, 1.92735333, 2.63291662, 1.58723604, 1.40951372]
    ...
    # with open("data/vqa_energy_" + str(n_qubits) + "qubits_v2.txt", 'w') as file:
